  ...
  class MongoStore extends Store {
    ...
    setAutoRemoveAsync() {
      ...
      switch (this.autoRemove) {
        case 'native':
          return this.collection.ensureIndexAsync({
            expires: 1
          }, {
    ...
    setCollection(collection) {
      ...
      ['count', 'findOne', 'remove', 'drop', 'ensureIndex'].forEach(method => {
        collection[method + 'Async'] = Promise.promisify(collection[method], {
          context: collection
        });
      });
      collection.updateAsync = Promise.promisify(collection.update, {
        context: collection,
        multiArgs: true
      });
    ...
    get(sid, callback) {
      ...
      return this.collectionReady()
        ...
        .then(session => {
          ...
          callback && typeof callback === 'function' && callback(err, sess);
        });
    }
    ...
    destroy(sid, callback) {
      const startTime = Date.now();
      return this.collectionReady()
        .then(collection => collection.removeAsync({
          _id: this.computeStorageId(sid)
        }))
